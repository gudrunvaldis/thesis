...
#svmmm = open("svm_dada6_bebe12_largebuckets.txt", "w") #svm_bebe12_dada6_sent_largebuckets
#lstmmm = open("lstm_dada6_bebe12_largebuckets.txt", "w") #lstm_bebe12_dada6_sent_largebuckets
onmm = open("oned_bebe12_dada12_sent_largebuckets.txt", "w") # oned_bebe12_dada6_sent_largebuckets
...
for x in range(0, 100):
	...
	a = np.loadtxt('bebe12_sent_largebuckets.txt')
	a_rows = a.shape[0]
	y1 = np.zeros(a_rows)

	b = np.loadtxt('dada12_sent_largebuckets.txt') #dada6_sent_largebuckets
	b_rows = b.shape[0]
	y2 = np.ones(b_rows)
	...
	#sys.exit() # MAKES THE PROGRAM END HERE
...
#	model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy']) #categorical_crossentropy!!!!! binary_crossentropy
#	print(model.summary())
#	model.fit(X_train, y_train, validation_data=(X_test, y_test), epochs=170, batch_size=64) # epochs=170 gives ~92-95%
